...
class RegisterForm(FlaskForm):
    ...
    password = PasswordField('Password', validators=[InputRequired('A password is required.'), EqualTo('confirm', message='Password should match')])
    confirm = PasswordField('Repeat password')
    image = FileField(validators=[FileAllowed(IMAGES, 'Only images are accepted.'), DataRequired()])
