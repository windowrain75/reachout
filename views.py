...
def register():
    ...
    if form.validate_on_submit():
        user_email = User.query.filter_by(username=form.username.data).first()
        if user_email:
            message = 'User already exists, enter a different username'
            return render_template('register.html', form = form, message=message)

        image_filename = photos.save(form.image.data)
        image_url = photos.url(image_filename)

        new_user = User(name=form.name.data, username=form.username.data, image=image_url, \
                        password=form.password.data, join_date=datetime.now()) #password=generate_password_hash(form.password.data)
